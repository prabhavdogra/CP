...
NODE apply(UPDATE l, NODE r) {
    NODE res = r;
    if(l.first != -1)
        res.first = l.first * r.second;
    res.first += l.second * r.second;
    return res;
}
UPDATE id() { return UPDATE(-1, 0); }
UPDATE combine(UPDATE cur, UPDATE prev) { 
    UPDATE res = cur;
    if(prev.first == -1)
        if(cur.first == -1) res = UPDATE(-1, cur.second + prev.second);
        else res = UPDATE(cur);
    else
        if(cur.first == -1) res = UPDATE(prev.first, cur.second + prev.second);
        else res = UPDATE(cur);
    return res;
}
...
signed main() {   
	...
	int n, q;
	cin >> n >> q;
	vector<pair<int, int>> a(n, pair<int, int>{0, 1});
    ...
    while(q--) {
        ...
        if(type == 1 || type == 2)
            cin >> v;
        if(type == 1)
            st.rupd(l, r - 1, UPDATE(v, 0));
        else if(type == 2)
            st.rupd(l, r - 1, UPDATE(-1, v));
        else
            cout << st.query(l, r - 1).first << '\n';
    }
    return 0;
}
