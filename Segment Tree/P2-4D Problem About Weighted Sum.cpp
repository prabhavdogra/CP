...
using NODE2 = pair<int, int>;
using UPDATE2 = int;

NODE2 initialization2() { return NODE2(0, 0); }
NODE2 operation2(NODE2 l, NODE2 r) {
    return NODE2(l.first + r.first, l.second + r.second);
}
NODE2 apply2(UPDATE2 l, NODE2 r) {
    return NODE2(r.first + l * r.second, r.second);
}
...
signed main() {
    ...
    vector<pair<int, int>> b(n, pair<int, int>{0, 1});
    for(int i = 0; i < n; i++) {
        b[i].first = a[i].first;
        a[i].first = (i + 1) * a[i].first;
    }
	lazy_segtree<NODE, operation, initialization, UPDATE, apply, combine, id> st(a);
    lazy_segtree<NODE2, operation2, initialization2, UPDATE2, apply2, combine2, id2> st2(b);
    while(q--) {
        int type, l, r, d;
        cin >> type >> l >> r;
        if(type == 1) {
            ...
            st2.rupd(l - 1, r - 1, UPDATE(d));
        }
        else {
            int val = st.query(l - 1, r - 1).first;
            val -= (l - 1) * st2.query(l - 1, r - 1).first;
            cout << val << '\n';
        }
    }
    return 0;
